...
const meta = computed(() => item.value?.metadata || {})
const settings = computed(() => generation.value?.settings || {})
...
const allSettings = computed(() => {
  ...
  if (!Object.keys(s).length && gs) {
    Object.entries(gs).forEach(([k, v]) => { if (v != null) s[k] = v })
  }
  return s
})
    ...
    <template v-else-if="data && item">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-5">
          ...
          <details v-if="item.metadata" class="group">
            ...
            <pre class="text-[10px] text-slate-600 bg-slate-50 rounded-lg p-3 border border-slate-100 mt-1.5 overflow-x-auto max-h-60">{{ JSON.stringify(item.metadata, null, 2) }}</pre>
          </details>
        </div>
      </div>
    </template>
