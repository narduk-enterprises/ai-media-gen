...
export async function submitJob(
  ...
  const response = await ofetch<MultiSegmentResponse>(`${url}/generate/multi-segment`, {
    method: 'POST',
...
export async function checkJobStatus(
  ...
  try {
    const status = await ofetch<GenerationStatus & { has_video?: boolean; has_image?: boolean }>(`${url}/generate/status/${jobId}`, {
      timeout: 10_000,
    })
    ...
    if (status.status === 'completed') {
      ...
      if (!data && (status.has_video || status.has_image)) {
        try {
          const result = await ofetch<{ video_base64?: string; image_base64?: string }>(`${url}/generate/result/${jobId}`, {
            timeout: 180_000,
          })
...
export async function getJobProgress(
  ...
  try {
    return await ofetch<GenerationStatus>(`${url}/generate/status/${jobId}`, {
      timeout: 10_000,
    })
  } catch {
    return null
...
export async function getHealth(podUrl?: string): Promise<HealthResponse | null> {
  ...
  try {
    return await ofetch<HealthResponse>(`${url}/health`, { timeout: 8_000 })
  } catch {
    return null
  }
}
...
export async function listModels(podUrl?: string): Promise<Record<string, ModelFile[]> | null> {
  ...
  try {
    return await ofetch<Record<string, ModelFile[]>>(`${url}/models`, { timeout: 8_000 })
  } catch {
    return null
  }
}
...
export async function getLogs(
  ...
  try {
    const result = await ofetch<{ source: string; lines: string }>(`${url}/logs`, {
      params: { source, lines },
      timeout: 5_000,
    })
    return result.lines
...
export async function restartComfyUI(podUrl?: string): Promise<{ success: boolean; output: string }> {
  ...
  return await ofetch<{ success: boolean; output: string }>(`${url}/restart`, {
    method: 'POST',
    timeout: 15_000,
  })
}
...
export async function startModelSync(podUrl?: string): Promise<{ status: string }> {
  ...
  return await ofetch<{ status: string }>(`${url}/sync-models`, {
    method: 'POST',
    timeout: 10_000,
  })
}
...
export async function getSyncStatus(podUrl?: string): Promise<SyncState | null> {
  ...
  try {
    return await ofetch<SyncState>(`${url}/sync-status`, { timeout: 5_000 })
  } catch {
    return null
  }
}
