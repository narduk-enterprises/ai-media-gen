...
<template>
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    <h1 class="font-display text-2xl sm:text-3xl font-bold text-white/90 mb-8">Settings</h1>
    ...
    <UCard class="mb-6" variant="outline">
      <div class="flex flex-wrap items-center gap-x-8 gap-y-4">
        ...
        <UFormField label="Name" size="sm">
          <p class="text-sm text-white/80">{{ user?.name || 'Not set' }}</p>
        </UFormField>
      </div>
    </UCard>
    ...
    <UCard class="mb-6" variant="outline">
      <template #header>
        <div class="flex items-center justify-between">
          <h2 class="text-sm font-semibold text-white/40 uppercase tracking-wider">GPU Pods</h2>
      ...
      <div v-else class="space-y-2">
        <div v-for="pod in activePods.pods" :key="pod.id" class="flex items-center justify-between p-3 rounded-lg bg-white/3 border border-white/6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-white/80">{{ pod.name || pod.id }}</p>
      ...
      <div class="mt-5 pt-4 border-t border-white/6">
        <div class="flex items-center justify-between mb-2">
          <div>
            <p class="text-xs font-medium text-white/70">Recover Lost Generations</p>
            <p class="text-[10px] text-white/30">Re-poll the API for any pending jobs that may have completed.</p>
          </div>
          <UButton size="xs" variant="outline" color="neutral" :loading="recovering" @click="recoverGenerations">
            Recover
          </UButton>
        </div>
        <div v-if="recoverResult" class="mt-2 p-3 rounded-lg text-xs" :class="recoverResult.recovered > 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-white/3 text-white/50'">
        ...
        <div v-if="recoverError" class="mt-2 p-3 rounded-lg bg-red-500/10 text-red-400 text-xs">
          {{ recoverError }}
        </div>
      </div>
    </UCard>
    ...
    <UCard class="border-red-100" variant="outline">
      <h2 class="text-sm font-semibold text-white/40 uppercase tracking-wider mb-4">Session</h2>
