...
const {
  ...
  fetch: fetchVideos,
  loadMore,
} = useRecentVideos(props.pageSize);
...
const previewUrl = computed(() => {
  if (selectedId.value)
    return videos.value.find((i: { id: string }) => i.id === selectedId.value)?.url || '';
...
function selectAll() {
  const ids = new Set<string>(videos.value.map((i: { id: string }) => i.id));
  selectedIds.value = ids;
  emit('update:selected', Array.from(ids));
}
...
<template>
  <div class="space-y-3">
    ...
    <div
      v-if="!multi && previewUrl"
      class="flex items-center gap-3 p-2 rounded-lg bg-primary-50 border border-primary-200"
    >
      <video
        :src="previewUrl + '#t=0.1'"
        class="w-14 h-14 rounded-lg object-cover shrink-0 border border-primary-200"
        muted
        playsinline
      />
    ...
    <div
      v-if="multi && selectedCount > 0"
      class="flex items-center gap-2 p-2 rounded-lg bg-primary-50 border border-primary-200"
    >
    ...
    <div
      v-if="showUpload && !multi && !previewUrl"
    ...
    <section>
      <div class="flex items-center justify-between mb-2">
        <label class="text-[11px] font-medium text-gray-500 uppercase tracking-wider">{{
          label
        }}</label>
        <div class="flex items-center gap-1">
          <UButton v-if="multi" size="xs" variant="ghost" color="primary" @click="selectAll"
            >All</UButton
          >
          <UButton
            size="xs"
      ...
      <div
        v-if="loadingVideos && videos.length === 0"
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        >
          ...
          />
          <div
            v-if="isSelected(vid.id)"
            class="absolute inset-0 bg-primary-400/20 flex items-center justify-center"
          >
      ...
      <p v-if="!loadingVideos && videos.length === 0" class="text-xs text-gray-400 py-2">
        No videos found yet.
      </p>
    </section>
  </div>
