 ...
 *
 * Uses the flat media-item pagination from GET /api/generations?type=video.
 */
export function useRecentVideos(pageSize = 20) {
  const videos = ref<{ id: string; url: string; prompt: string }[]>([])
  ...
  async function fetchPage() {
    const data = await $fetch<{
      ...
      params: { limit: pageSize, offset: currentOffset, type: 'video' },
  ...
  async function loadMore() {
    ...
    try {
      const result = await fetchPage()
      videos.value = [...videos.value, ...result]
    } catch {}
    loading.value = false
  }

  return { videos, loading, hasMore, fetch, loadMore }
}
